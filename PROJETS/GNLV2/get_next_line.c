...
/*   Updated: 2024/03/25 20:14:31 by mafourni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char 	*read_fd(int fd, char *keep_line, char *buffer_clean)
{
	...
	{	
		...
		{
			buffer_clean = malloc(sizeof(char) * ft_strlen(ft_strchr(keep_line, '\n')) + 1);
			if (!buffer_clean)
				return (NULL);
			len = ft_strlen(ft_strchr(keep_line, '\n'));
			ft_memmove(buffer_clean, ft_strchr(keep_line, '\n'), len + 1) ;
			break;
		}
		check_read = read(fd, buffer, BUFFER_SIZE);
	}
